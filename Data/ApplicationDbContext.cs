...
public class ApplicationDbContext : IdentityDbContext<ApplicationUser>
{
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        builder.Entity<SupplierReceipt>().HasQueryFilter(x=>!x.IsDeleted);builder.Entity<SupplierPayment>().HasQueryFilter(x=>!x.IsDeleted);builder.Entity<SupplierCredit>().HasQueryFilter(x=>!x.IsDeleted);builder.Entity<CustomerDelivery>().HasQueryFilter(x=>!x.IsDeleted);builder.Entity<CustomerPayment>().HasQueryFilter(x=>!x.IsDeleted);
        ...
        foreach(var type in new[]{typeof(SupplierReceipt),typeof(SupplierPayment),typeof(SupplierCredit),typeof(CustomerDelivery),typeof(CustomerPayment)})
        {
        ...
        builder.Entity<SupplierPayment>().HasOne(x=>x.SupplierCredit).WithMany().OnDelete(DeleteBehavior.Restrict);
        builder.Entity<SupplierCredit>().HasOne(x=>x.SourceSupplierReceipt).WithMany().OnDelete(DeleteBehavior.Restrict);
        builder.Entity<SupplierCredit>().HasOne(x=>x.Supplier).WithMany().OnDelete(DeleteBehavior.Restrict);
        builder.Entity<SupplierCredit>().HasOne(x=>x.PaymentMethod).WithMany().OnDelete(DeleteBehavior.Restrict);
        builder.Entity<SupplierCredit>().HasIndex(x=>new{x.SupplierId,x.RemainingAmount});
        builder.Entity<SupplierCredit>().HasIndex(x=>x.SourcePaymentGroupId).IsUnique().HasFilter("[IsDeleted] = 0");
