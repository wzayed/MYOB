...
public class AuditTrailModel(IAuditReportService reports,ApplicationDbContext db):PageModel
{
    ...
    public async Task OnGetAsync(){var all=ModelState.IsValid?await reports.GetAsync(Filter):[];const int size=50;TotalPages=Math.Max(1,(int)Math.Ceiling(all.Count/(double)size));PageNumber=Math.Clamp(PageNumber,1,TotalPages);Rows=all.Skip((PageNumber-1)*size).Take(size).ToList();Users=await db.AuditLogs.AsNoTracking().Select(x=>x.UserName).Distinct().OrderBy(x=>x).ToListAsync();}
    public async Task<IActionResult> OnGetPdfAsync(){if(!ModelState.IsValid)return BadRequest("حدد الفترة بصورة صحيحة ولا تستخدم تاريخاً بعد اليوم.");var rows=await reports.GetAsync(Filter);var bytes=new AuditTrailPdfDocument(rows,Filter).GeneratePdf();return File(bytes,"application/pdf",$"audit-trail-{DateTime.Now:yyyyMMddHHmm}.pdf");}
}
